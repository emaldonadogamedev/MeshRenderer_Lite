...
bool IEngine::Initialize(HINSTANCE hInstance)
{
	//TODO:
...
ISystem* IEngine::GetSystem(const SystemType systemType) const
{
	return m_systems[(int)systemType];
}

void IEngine::SendMessageToSystems(const IMessage * const msg)
{
	if (msg)
	{
		for (auto& it : m_systems)
...
bool IEngine::IsRunning() const
{
	return m_isRunning;
}

extern LRESULT ImGui_ImplDX11_WndProcHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);
LRESULT IEngine::MessageHandler(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
...
void IEngine::OnMouseDown(WPARAM buttonState, int x, int y)
{

}

void IEngine::OnMouseUp(WPARAM buttonState, int x, int y)
{

}

void IEngine::OnMouseMove(WPARAM buttonState, int x, int y)
{

}

const GameClock& IEngine::GetClock(void) const
{
	return *m_clock;
}

IEngine* IEngine::s_engineApp;

bool IEngine::AddSystemHelper(ISystem* const sys /*= nullptr*/)
{
	if (!sys)
		return false;
...
void IEngine::CalculateFrameTime(void)
{
	m_frameCount++;
...
const float IEngine::s_fps60 = 1.0f / 60.0f;