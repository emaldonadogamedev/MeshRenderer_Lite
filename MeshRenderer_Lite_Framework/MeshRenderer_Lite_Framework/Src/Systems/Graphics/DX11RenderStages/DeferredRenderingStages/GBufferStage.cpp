...
GBufferStage::GBufferStage(DX11Renderer* const renderer, RenderCompUmap* const gfxComponents, const Model* const boxGeometry)
...
void GBufferStage::Render(HandleDictionaryVec& graphicsResources, const float dt)
{
	...
	{
		handle = (graphicsResources[(int)ObjectType::VERTEX_SHADER]).at("SkyBoxVS");
		m_renderer->BindVertexShader(handle);
		
		//Need the depth function LESS_EQUAL for the skybox
		ID3D11DepthStencilState* currDepthStencilState;
		UINT currStencilRef;
		m_renderData.m_pImmediateContext->OMGetDepthStencilState(&currDepthStencilState, &currStencilRef);
		...
		m_renderData.testPerObjectBuffer.isAnimated = false;
		m_renderData.testPerObjectBuffer.isSkydome = true;
		...
		m_renderData.m_pImmediateContext->PSSetShaderResources(0, 1, &m_renderData.textures2D[*handle].srv);
		m_renderer->DrawIndexed(m_boxModel->GetIndicesCount());

		//revert changes for the other objects
		m_renderData.testPerObjectBuffer.isSkydome = false;
		m_renderData.m_pImmediateContext->OMSetDepthStencilState(currDepthStencilState, currStencilRef);
	}

	m_renderData.m_pImmediateContext->RSSetState(m_renderData.m_d3dRasterStateSolCullNone);
	
	//Set the regular vertex shader
