...
{
	RenderTarget bloomRT = m_renderData.renderTargets[*m_renderData.m_BloomBrightMap];
	m_passRTs[1] = bloomRT.rtv;
}
...
void DeferredSimpleLightStage::PreRender()
{
	...
	//clear the bloom map RT
	m_renderer->ClearRenderTarget(m_renderData.m_BloomBrightMap, DirectX::Colors::Black.v);
	
	//bind the 2 render targets
	const auto& renderTarget = m_renderData.renderTargets[ *m_renderData.m_MainRenderTargets[m_renderData.m_currentMainRTindex] ];
	m_passRTs[0] = renderTarget.rtv;

	m_renderData.m_pImmediateContext->OMSetRenderTargets(2, m_passRTs, nullptr);
}
